...
func GenForwardRollbackSqlFromBinEvent(i uint, cfg *ConfCmd, wg *sync.WaitGroup) {
	...
	lastOrgSql := ""
	original_sql_file, err := os.OpenFile(cfg.OutputDir+"/original_sql.sql", os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0644)
	if err != nil {
	}
	defer original_sql_file.Close()
	buf_original_sql_file := bufio.NewWriter(original_sql_file)
	defer buf_original_sql_file.Flush()
...
func GetForwardRollbackContentLineWithExtra(sq ForwardRollbackSqlOfPrint, ifExtra bool) string {
	if ifExtra {
		return fmt.Sprintf("-- datetime=%s database=%s table=%s binlog=%s startpos=%d stoppos=%d gtid=%s orgSql=%s\n%s;\n",
			sq.sqlInfo.datetime, sq.sqlInfo.schema, sq.sqlInfo.table, sq.sqlInfo.binlog, sq.sqlInfo.startpos,
			sq.sqlInfo.endpos, sq.sqlInfo.gtid, strings.ReplaceAll(sq.sqlInfo.orgSql, "\n", "\n-- "), strings.Join(sq.sqls, ";\n"))
	} else {
