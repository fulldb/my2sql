...
func (this *ConfCmd) ParseCmdOptions() {
	...
	flag.BoolVar(&this.FilePerTable, "file-per-table", true, "One file for one table if true, else one file for all tables. default true. Attention, always one file for one binlog")
	...
	flag.BoolVar(&this.FilePerTable, "file-per-gtid", false, "One file for one gtid if true, else one file for all gtid. default false. Attention, always one file for one binlog")
	flag.StringVar(&this.Gtid, "gtid", "", "one gtid")
	flag.StringVar(&this.Sqltext, "sqltext", "", "like sqltext")
